...
class _CourseEditorState extends State<CourseEditor>
                      ...
                      (module['lessons'] as List<dynamic>?)
                          ?.map(
                            (lesson) => {
                              ...
                              'documents':
                                  lesson['documents'] != null
                                      ? (lesson['documents'] as List<dynamic>)
                                          .map(
                                            (doc) =>
                                                doc is String
                                                    ? PlatformFile(
  ...
  void _addLesson(int moduleIndex) {
    setState(() {
      ...
      print(
        'Added lesson to module $moduleIndex: ${_modules[moduleIndex]['lessons'].last}',
      ); // Debug
      _animationController.forward(from: 0);
    });
  }
    ...
    try {
      ...
      for (var module in _modules) {
        ...
        for (var lesson in module['lessons']) {
          ...
          for (var file in (lesson['documents'] as List<PlatformFile>?) ?? []) {
          ...
          for (var file in (lesson['videos'] as List<PlatformFile>?) ?? []) {
            if (file.bytes != null || file.path != null) {
              final url = await _uploadFile(
                file,
                'video',
          ...
          for (var file in (lesson['images'] as List<PlatformFile>?) ?? []) {
            if (file.bytes != null || file.path != null) {
              final url = await _uploadFile(
                file,
                'image',
      ...
      children: [
        ...
        _buildFilePicker(
          ...
          onTap: () async {
            ...
            if (files.isNotEmpty) {
              setState(
                () => lesson['documents'] = [...lesson['documents'], ...files],
              );
            }
          },
          files: lesson['documents'] as List<PlatformFile>? ?? <PlatformFile>[],
          fileType: 'Documents',
          allowedExtensions: ['pdf', 'doc', 'docx'],
        ),
        _buildFilePicker(
          ...
          onTap: () async {
            ...
            if (files.isNotEmpty) {
              setState(
                () => lesson['videos'] = [...lesson['videos'], ...files],
              );
            }
          },
          files: lesson['videos'] as List<PlatformFile>? ?? <PlatformFile>[],
          fileType: 'Videos',
        ...
        _buildFilePicker(
          ...
          onTap: () async {
            ...
            if (files.isNotEmpty) {
              setState(
                () => lesson['images'] = [...lesson['images'], ...files],
              );
            }
          },
          files: lesson['images'] as List<PlatformFile>? ?? <PlatformFile>[],
          fileType: 'Images',
          allowedExtensions: ['jpg', 'jpeg', 'png'],
        ),
      ],
